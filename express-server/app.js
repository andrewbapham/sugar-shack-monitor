...
let portName = await new Promise(resolve => {
    rl.question(`Which serial port would you like to use? The first option will be selected by default in ${SP_PROMPT_TIMEOUT / 1000} seconds.\n`, { signal }, resolve)
    signal.addEventListener('abort', async () => {
        console.log("Serial port selection timeout expired. Defaulting to first available serial port.");
